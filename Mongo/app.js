...
app.get('/user/:id', (req, res) => {
    if (ObjectId.isValid(req.params.id)) {
        db.collection('user')
            ...
            .catch((err) => {
                res.status(404).json({ error: 'Couldn\'t not find the user' })
            })
    }
    else {
...
app.delete('/user/:id', (req, res) => {
    if (ObjectId.isValid(req.params.id)) {
        db.collection('user')
            ...
            .catch((err) => {
                res.status(500).json({ error: 'Couldn\'t not delete the user' })
            })
    }
    else {
        res.status(500).json({ error: 'Invalid Document Id' })
...
app.put('/user/:id', (req, res) => {
    ...
    if (ObjectId.isValid(req.params.id)) {
        db.collection('user')
            ...
            .catch((err) => {
                res.status(500).json({ error: 'Couldn\'t not update the user' })
            })
    }
    else {
        res.status(500).json({ error: 'Invalid Document Id' })
